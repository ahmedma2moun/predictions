...
export async function sendKickoffReminderEmail(to: string | null | undefined, match: MatchForEmail, kind: 'before' | 'kickoff' = 'before'): Promise<void> {
  if (!to) return;

  const heading = kind === 'kickoff' ? 'Kickoff is now!' : 'Kickoff in 60 minutes!';
  const subject = kind === 'kickoff' ? `Kickoff now: ${match.homeTeamName} vs ${match.awayTeamName}` : `Kickoff in 60 min: ${match.homeTeamName} vs ${match.awayTeamName}`;
    ...
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#1a1a1a;">
      ...
      <div style="padding:24px;border:1px solid #e5e5e5;border-top:none;border-radius:0 0 8px 8px;">
        ...
        <div style="margin-bottom:20px;padding:10px 14px;background:#eff6ff;border-radius:6px;border-left:4px solid #3b82f6;font-size:13px;color:#1e3a8a;">
          ${kind === 'kickoff' ? 'The match is starting now.' : 'Predictions lock at kickoff — last chance to submit or double-check yours!'}
        </div>
        <div style="text-align:center;">
