...
| Football provider | TheSportsDB active (`FOOTBALL_PROVIDER=thesportsdb`) | Requires a paid `THESPORTSDB_API_KEY` (already configured) — see `docs/football-providers/thesportsdb.md` |
| Match events (goals/cards) | Embedded in `APIFixture.events`, fetched inside `fetchFixtureById()` | Reuses the existing live-score call/cache instead of a new provider method/route; TheSportsDB populates via `/lookuptimeline.php` for finished/live fixtures, other providers return `[]`. TheSportsDB's own event-timeline coverage is competition-dependent — e.g. it returns no data at all for UEFA Super Cup despite reporting a live score — so the events card is expected to be empty for some competitions |
...
| TheSportsDB live score source | `/livescore/{leagueId}` overrides `/lookup/event/{id}`'s status/score while a match is live | `/lookup/event` lags real match state (confirmed: reported `HT` while `/livescore` already showed `2H` for the same fixture); `/livescore` is TheSportsDB's dedicated live feed — see ADR-17, `SYSTEM_ARCHITECTURE.md` |
