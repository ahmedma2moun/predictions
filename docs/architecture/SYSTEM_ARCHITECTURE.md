...
### ADR-13: `Match.status = 'live'` written opportunistically from live-score polling and `fetch-results`, not on a dedicated schedule
**Decision**: Added `syncMatchStatus()` (`src/lib/services/match-service.ts`), called from both `/api/matches/[matchId]/live` and `/api/mobile/matches/[matchId]/live` after every poll, and inlined into `processMatchResults()`'s per-match loop (`src/lib/results-processor.ts`) — both write `Match.status = 'live'` the moment `mapFixtureStatus()` reports `'live'` for a match whose DB status isn't already `'live'`.
**Rationale**: `Match.status` was only ever written in two places — `matches-processor.ts` at insert time (always `'scheduled'`) and `results-processor.ts` once a result comes in (`'finished'`) — so it silently stayed `'scheduled'` for a match's entire duration in play. Every `match.status === 'live'` UI check (the LIVE pill/badge on the matches list and match detail pages, web and mobile) depended on this field, so none of them could ever fire; a live match would show "LOCKED" instead of "LIVE" even while its score was correctly polling and updating (that score comes from a separate, already-working code path — the `/live` routes' response body — not from `Match.status`). Piggybacking on the existing `/live` poll gives near-real-time correction for anyone actively viewing a match; piggybacking on `fetch-results` (which already fetches each unfinished match's current API status every run) catches matches nobody has open, so the matches-list badge is correct on next load without needing a new cron.
